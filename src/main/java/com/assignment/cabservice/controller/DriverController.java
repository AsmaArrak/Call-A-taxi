...
public class DriverController {
    ...
    public String showNewDriverPage(Model model) {
        ...
        model.addAttribute("driver", new Driver());  // Add an empty Driver object to the model
        model.addAttribute("carIds", cars);
        return "driver";
    }
    ...
    //public String addNewTodo(@Valid Todo todo, ModelMap modelMap, BindingResult bindingResult) {
        ...
        public String addNewDriver(@ModelAttribute("driver") Driver driver) {
            // Set password (you may want to handle this differently)
            driver.setPassword("$2a$12$TLJOLK.QjLRdxOHew1XMT.eYa2Xr5HMHaT14fRoI3gMOIZijNu9F2");
        
            // Set usedCarIds with the selected carId
            driver.setUsedCarIds(String.valueOf(driver.getAssignedCarId()));
        
            // Save the driver
            Driver savedDriver = driverRepository.save(driver);
        
            // Update the corresponding car with the driverId
            Optional<Car> optionalCar = carRepository.findById(driver.getAssignedCarId());
